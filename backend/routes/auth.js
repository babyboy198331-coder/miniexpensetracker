...
router.post("/refresh", async (req, res) => {
  ...
  if (!(await store.isRefreshTokenValid(refreshToken))) {
...
router.post("/forgot-password", async (req, res) => {
  ...
  await store.saveResetToken(resetToken, user.id, Date.now() + RESET_TOKEN_TTL_MS);
...
router.post("/reset-password", async (req, res) => {
  ...
  const entry = await store.findResetToken(token);
  if (!entry) {
  ...
  await store.revokeAllRefreshTokensForUser(user.id);
